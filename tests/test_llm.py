...
def chunk(content="", reasoning="", finish=None):
...
class LLMTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reasoning_is_not_used_as_the_final_answer(self):
        ...
        thinking, answer = await llm.ask("s", "u", model="unit-test", max_tokens=32, retries=2)
    ...
    async def test_true_empty_response_is_explicit(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "no final answer"):
            await llm.ask("s", "u", model="unit-test", retries=1)
    ...
    async def test_think_only_content_is_not_an_answer(self):
        ...
        llm._client = SimpleNamespace(chat=SimpleNamespace(completions=completions))
        self.assertEqual((await llm.ask("s", "u", model="unit-test", retries=2))[1], "done")
    ...
    async def test_alternate_endpoint_uses_streaming(self):
        completions = FakeCompletions([FakeStream([
            chunk(reasoning="brief"), chunk("<answer>"),
            chunk("done", finish="stop"), chunk("</answer>"),
        ])])
        old_alternate = llm._alternate
        llm._alternate = lambda _model: True
        llm._new_client = SimpleNamespace(chat=SimpleNamespace(completions=completions))
        try:
            thinking, answer = await llm.ask("s", "u", "gpt-test", retries=1)
        finally:
            llm._alternate = old_alternate
        self.assertEqual((thinking, answer), ("brief", "done"))

    async def test_deepseek_uses_streaming_keepalive(self):
        completions = FakeCompletions([FakeStream([
            chunk(reasoning="brief"), chunk('<answer>{"ok":true}</answer>', finish="stop"),
        ])])
        llm._client = SimpleNamespace(chat=SimpleNamespace(completions=completions))
        thinking, answer = await llm.ask("s", "u", "deepseek-v4-pro", retries=1)
        self.assertEqual((thinking, answer), ("brief", '{"ok":true}'))
