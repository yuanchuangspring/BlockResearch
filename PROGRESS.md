...
### 2026-07-18 结构精简后五轮盲测

| 轮次 | 结果 | 用时 | 阶段 | 正确候选召回 | 主要验证 |
|---|---:|---:|---:|---:|---|
| 1 | 1/1 | 513s | 6 | S3 | 稳定候选可提前停止 |
| 2 | 1/1 | 232s | 3 | S1 | 精简幕僚摘要后快速闭环 |
| 3 | 0/1 | 355s | 5 | 未召回 | 暴露 GPT 长请求连接错误与信息丢失 |
| 4 | 0/1 | 746s | 8 | 未召回 | 无连接错误，仍暴露候选召回方差 |
| 5 | 0/1 | 670s | 8 | 未召回 | 动态 SOLVE→SEARCH 和 SOLVE→VERIFY 信息流已打通，但正确候选仍未出现 |

聚合：2/5 = 40%，平均 503s/题，正确候选召回 2/5；两题一旦召回正确候选均最终答对，三道错题均是召回失败。
...
- [x] 将已查看开发题的评测从“最终答案是否出现”扩展为候选、中间实体/文档/事件桥、SERP 排名与正文命中分层评测
- [x] 查明 Brave 空结果不是解析 bug：旧 key 返回 402；新 key 认证 200 但响应头显示月度配额/remaining 均为 0，Web Search 无 `web.results`，LLM Context 未订阅
- [x] 高召回规划改为 3 条独立路线：源文档锨点 → 中间桥 → 候选 join；禁止把距离/数值范围等派生条件当原文搜索
- [x] 对间接未知值保留多个桥候选，禁止收缩为单个著名样例；论文/备案/演员表等先搜稳定字段结构
- [x] 确定性查询编译器：3+ 引号关系的关键词袋拆为原子查询，批次最多 16 条；开发题过度约束查询降为 0
- [x] 离线规划复测：Leeds 种子 rank 6、桥 3/3；El Wak rank 2、桥 4/4；KT Tunstall rank 8；Ross Andel 桥 3/3（候选待在线结构查询召回）
- [x] 历史 URL 抓取重放 20/20 成功；passage 排名改为密集覆盖段优先、稀有长词优先、相近窗口去重
- [ ] 待有月度配额的 Brave Web Search key 后，重放五题真实 SERP 排名与抓取桥闭环
...
- [x] SERP 按查询路线等额压缩后再交给精炼器，避免上下文截断使后半路线不可见
- [x] 中间实体改为临时图前沿：`已取回节点 -> 未解决边 -> 目标节点类型`，下一跳必须从真实结果节点出发
- [x] Research State 在固定 12 条 lead 预算内按查询路线保留前沿，防止并行搜索互相覆盖
- [x] 查询回显检测覆盖 URL path 与 query string，降低关键词灌入式搜索页排名
- [x] 删除针对开发错题类型的提示词措辞，只保留关系图遍历、路线覆盖和禁止猜测未知节点等通用原则
- [x] 54/54 回归测试通过
- [x] 新隔离题盲测：1/1，正确候选 S1 召回，421 秒，5 阶段；未查看或保存隔离题内容轨迹
- [ ] 新暴露的通用效率问题：正确候选已早期召回，但完整身份链 Verifier 连续拒绝，使 Builder 重复补证；后续需统一停止与验证契约

- [x] 执行链结构性瘦身：常规阶段从 `Builder → Selector → Fetch → Auditor → Solver` 缩为 `Builder → Search → 确定性分散 Fetch → Solver`
- [x] 删除常驻 LLM URL Selector；按搜索排名、query-echo 过滤和域名去重自动抓取
- [x] 删除常驻 Auditor LLM；专业 Solver 输出原子 claim+引句，Notebook 仍机械校验引句与来源后才升级为 verified
- [x] GPT 大请求超时快速失败：同时关闭项目外层和 OpenAI SDK 内层隐式重试，修复单次 90s 实际占用 274s；45/45 回归测试通过
- [x] 当前 GPT 角色统一使用 `reasoning_effort=low`，优先降低研究循环的单步延迟和费用
- [x] 恢复原始角色边界：Builder 是研究负责人；SOLVE/VERIFY 均为 Builder 可选放入图中的幕僚，框架不再强制补入或图外调用
- [x] Solver 输出缩为 memo/claims/candidates/decisive_gap/recommendation/best_guess；Research State 缩为确定证据、候选、决胜缺口、最近 Builder 决策和动作账本
- [x] 评测预算结束必须输出 best guess，Verifier 只标注证据是否闭环，不再将答案清空
